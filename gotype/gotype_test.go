...
func runTest(t *testing.T, path string) {
	...
	*allErrors = true
	...
	if suffix := ".go"; strings.HasSuffix(path, suffix) {
		// single file, part of this package so on GOPATH.
		path = filepath.Join(ctxt.GOPATH, "/src/", path)
...
var tests = []string{
	// individual files, part of this package.
	"code.google.com/p/go.exp/gotype/testdata/test1.go",
