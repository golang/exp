...
var (
	predefined = []string{"rules"} // hard-coded elements exist of the form <name>Elem
	skipElem   = []string{
...
var (
	...
	reAttr  = regexp.MustCompile(` *(\w+) *(?:(\w+)|\(([\w\- \|]+)\)) *(?:#([A-Z]*))? *("[\w\-:]*")?`)
...
// isLDML returns true if the builder is used for generating LDML XML,
// as opposed to any of the other auxiliary formats.
func (b *builder) isLDML() bool {
	return b.info.file == "ldml"
}
...
func (b *builder) parseDTD(r io.Reader) {
	for d := xml.NewDecoder(r); ; {
		...
		case "ATTLIST":
			...
			switch m[1] {
			case "draft", "references", "alt", "validSubLocales", "standard" /* deprecated */ :
			case "type", "choice":
			default:
...
func (b *builder) writeElem(tab int, e *element) {
	...
	for _, attr := range e.attr {
		if !in(skipAttr, attr.name) {
			p("\n%s string `xml:\"%s,attr\"`", title(attr.name), attr.name)
		}
	}
	for _, sub := range e.sub {
		if in(predefined, sub.e.name) {
			p("\n%sElem", sub.e.name)
			continue
		}
		if b.isLDML() && in(skipElem, sub.e.name) {
			continue
		}
		p("\n%s ", title(sub.e.name))
		if sub.repeat {
